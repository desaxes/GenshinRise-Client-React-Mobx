...
export const RollBanner = observer((props) => {
    ...
    const Rolls = rolls?.rolls.map(e => e.isChar ?
        <StyledImg height={app.game === 'Honkai' && '90px'} style={{ boxShadow: e.stars === 5 ? '0px 0px 10px 10px yellow' : (e.stars === 4 ? '0px 0px 15px 5px violet' : ''),border:'2px white solid' }} width='65' br={app.game !== 'Honkai' ? '50%' : '16px'} bg={e.stars === 5 ? 'orange' : (e.stars === 4 ? '#4600f6' : '#4682B4')} src={process.env.REACT_APP_API_URL + (app.game === 'Genshin' ? '/chars/' : (app.game === 'Zzz' ? '/zzz/chars/' : '/honkai/chars/')) + e.img} />
        :
        <StyledImg height={app.game === 'Honkai' && '90px'} style={{ boxShadow: e.stars === 5 ? '0px 0px 10px 10px yellow' : (e.stars === 4 ? '0px 0px 15px 5px violet' : ''),border:'2px white solid' }} width='65px' br={app.game !== 'Honkai' ? '50%' : '16px'} bg={e.stars === 5 ? 'orange' : (e.stars === 4 ? '#4600f6' : '#4682B4')} src={process.env.REACT_APP_API_URL + (app.game === 'Genshin' ? '/weapons/' : (app.game === 'Zzz' ? '/zzz/weapons/' : '/honkai/weapons/')) + e.img} />)
    const LegRolls = legs?.map(e => <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        {e.isChar ?
            <StyledImg style={{border:'2px white solid'}} width='65px' height={app.game === 'Honkai' && '90px'} br={app.game !== 'Honkai' ? '50%' : '16px'} bg={'orange'} src={process.env.REACT_APP_API_URL + (app.game === 'Genshin' ? '/chars/' : (app.game === 'Zzz' ? '/zzz/chars/' : '/honkai/chars/')) + e.img} />
            :
            <StyledImg style={{border:'2px white solid'}} width='65px' height={app.game === 'Honkai' && '90px'} br={app.game !== 'Honkai' ? '50%' : '16px'} bg={'orange'} src={process.env.REACT_APP_API_URL + (app.game === 'Genshin' ? '/weapons/' : (app.game === 'Zzz' ? '/zzz/weapons/' : '/honkai/weapons/')) + e.img} />}
